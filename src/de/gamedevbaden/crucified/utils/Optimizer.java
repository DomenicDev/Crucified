...
/**
 * A customized Optimizer class.
 *
 * Currently, I only "overwrote" the makeBatches() method to reapply the
 * render bucket after batching since this is not done automatically.
 *
 * @see GeometryBatchFactory
 */
public class Optimizer {
    ...
    public static void makeLods(Collection<Geometry> geometries, Mesh outMesh) {
        ...
        for (int i = 0; i < lodLevels; i++) {
            ...
            lods[i].setupData(VertexBuffer.Usage.Dynamic, 1, VertexBuffer.Format.UnsignedInt, BufferUtils.createIntBuffer(lodData[i]));
        }
    ...
    public static List<Geometry> makeBatches(Collection<Geometry> geometries, boolean useLods) {
        ...
        for (Map.Entry<Material, List<Geometry>> entry : matToGeom.entrySet()) {
            ...
            // if all geometries use the same bucket
            // we want to set the correct bucket after batching again
            RenderQueue.Bucket bucket = null;
            for (Geometry g : entry.getValue()) {
                if (bucket==null) {
                    // we set it the first time
                    bucket = g.getQueueBucket();
                } else {
                    // it has been set earlier
                    // we check if this geometry uses the same bucket
                    // if it does not we reset it to opaque and break out
                    if (g.getQueueBucket() != bucket) {
                        bucket = RenderQueue.Bucket.Opaque;
                        break;
                    }
                }
            }
     ...
     * @see GeometryBatchFactory.AlignOption
     *
